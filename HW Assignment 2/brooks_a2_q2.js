...
var listPlanets = function () {
  for (var i = 0; i < planets.length; i++) {
    document.write(planets[i].planet + ' is planet #' + planets[i].position + 
    	' from the Sun. Time to complete its orbit is ' + planets[i].orbit_time + ' earth year(s). It has '
       + planets[i].nat_satellites + ' natural satellite(s).<br>' );
    }
}
...
if (1 <= num && num <= 8) {
  // listPlanets();
  document.write(planets[num - 1].planet + ' is planet #' + planets[num - 1].position + 
    ' from the Sun. Time to complete its orbit is ' + planets[num - 1].orbit_time + ' earth year(s). It has '
     + planets[num - 1].nat_satellites + ' natural satellite(s).<br>' );
